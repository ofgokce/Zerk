...
**`Sendable` is required on the same terms as before, and no more.** The box coordinates callers through a `Task`, whose result the standard library requires to be `Sendable` — which would put a constraint on a kept instance that neither a `@Singleton`'s `static let` nor `ZerkScopedBox` imposes, so a type kept perfectly well today would stop being keepable the moment its provider gained an `async`. The box carries the value past that requirement instead. What remains is the rule that already applied: a kept instance crossing an isolation boundary must be `Sendable`, checked where it happens by [the conformance check Zerk emits](../Plugin/Diagnostics.md#the-one-diagnostic-that-is-not-zerks).
