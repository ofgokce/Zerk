...
| An `@injected` parameter in an extension names one of the extended type's generic parameters | A generic parameter is a different type at each call site while the generated overload resolves it once, so nothing can register one. Pass it in, or resolve it in the member's body. A *concrete* key is fine there — the generated extension repeats the header, so the type's parameters stay in scope for everything passed through |
| A provider parameter, or an unmarked parameter beside an `@injected` one, is variadic | The generated member has to pass it on, and Swift cannot: inside that member the parameter is an array and there is no way to spread one back into a variadic. Declare it as an array parameter |
