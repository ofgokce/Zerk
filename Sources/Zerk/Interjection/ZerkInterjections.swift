...
public final class ZerkInterjections: @unchecked Sendable {
    ...
    public static let processDefault = ZerkInterjections(allowsUnscopedRegistration: isRunningInPreview)
    ...
    public func interject(_ keyPath: AnyKeyPath, _ body: @escaping @Sendable () -> Any) {
        checkScoped()
        lock.lock()
        defer { lock.unlock() }
    ...
    public func interject(_ key: ObjectIdentifier, _ body: @escaping @Sendable () -> Any) {
        checkScoped()
        lock.lock()
        defer { lock.unlock() }
        byKey[key] = body
    ...
    /// exact.
    public func value<V>(for keyPath: AnyKeyPath,
...
public extension Zerk where T == Never {
    ...
    static func withInterjections<R>(_ operation: () async throws -> R) async rethrows -> R {
        try await ZerkInterjections.$current.withValue(ZerkInterjections(), operation: operation)
    }
    ...
    static func withInterjections<R>(_ operation: () throws -> R) rethrows -> R {
        try ZerkInterjections.$current.withValue(ZerkInterjections(), operation: operation)
    }
}
